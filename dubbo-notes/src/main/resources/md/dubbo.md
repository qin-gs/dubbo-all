...
- 本地存根
  ...
  在客户端也执行部分逻辑，比如：做 ThreadLocal 缓存，提前验证参数，调用失败后伪造容错数据等等

  提供 Stub 的实现，构造函数传入真正的远程代理对象

- 服务降级

  通过服务降级功能临时屏蔽某个出错的非关键服务，并定义降级后的返回策略

  - 消费方对该服务的方法调用都直接返回 null 值，不发起远程调用。用来屏蔽不重要服务不可用时对调用方的影响。
...
属性加载顺序    jvm -D    >    xml   >    properties

配置(timeout, retries, loadbalance, actives...)
...
1. 导入 dubbo-starter 在properties配置文件中配置属性，使用 @Service 暴露服务，@Reference 引用服务
...
1. 服务消费者以本地调用的方式调用服务
2. client stub 接收到调用后将方法，参数等组装成能够网络传输的消息
3. client stub 找到服务地址，将消息发送到服务端
4. server stub 接收到消息之后进行解码
5. server stub 根据解码结果调用本地服务
6. 本地服务执行后将结果返回 server stub
7. server stub 将结果打包成消息发送给消费方
8. client stub 接收到消息，进行解码
9. 服务消费方得到最终结果
...
NIO (non-blocking io)   Channel, Selector, IO多路复用

![Dubbo结构](./img/Dubbo结构.png)



xml 标签解析

DubboNamespaceHandler 注册标签解析器

![注册标签解析器](./img/注册标签解析器.png)

ServiceBean 负责暴露接口

ReferenceBean 负责引用服务(Invoker)



DubboBeanDefinitionParser 解析标签
